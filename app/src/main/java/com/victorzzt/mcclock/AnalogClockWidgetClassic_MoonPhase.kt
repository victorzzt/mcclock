...
class AnalogClockWidgetClassic_MoonPhase : AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        val itemRaw = BitmapFactory.decodeResource(context.resources, R.drawable.clock_mask_classic, options)
        
        // Determine lunar phase dial
        val moonPhase = ClockUtils.calculateLunarPhase().lowercase(Locale.US)

        val drawableName = "clock_dial_classic_$moonPhase"
        ...
        val dialRaw = if (dialResId != 0) {
            ...
            // Fallback to classic dial if specific moon phase asset is missing
            BitmapFactory.decodeResource(context.resources, R.drawable.clock_dial_classic, options)
        }
        ...
        for (appWidgetId in appWidgetIds) {
            val views = RemoteViews(context.packageName, R.layout.widget_analog_clock_classic_moonphase)
